<nav id="sidebar">
    ...
    <?php 
        if (file_exists('data/logo.png')) {
            echo '<a href="'.BASE_URL.'"><img id="logo" src="'.BASE_URL.'data/logo.png?'.time().'" alt="logo" class="img-fluid"></a>';
    ...
    <ul class="list-inline text-center">
        ...
        if (isset($_SESSION['Active'])) {
            echo '<li class="list-inline-item" data-toggle="tooltip" data-placement="top" title="'.$t->trans("Logout").'">
                    <a href="'.BASE_URL.'logout" id="sk-logout" class="btn btn-outline-secondary btn-sm" role="button"><i class="fa fa-sign-out" aria-hidden="true"></i></a>
                </li>';
        } else {
            echo '<li class="list-inline-item" data-toggle="tooltip" data-placement="top" title="'.$t->trans("Login").'">
                    <a href="'.BASE_URL.'login" id="sk-login" class="btn btn-outline-secondary btn-sm" role="button"><i class="fa fa-sign-in" aria-hidden="true"></i></a>
                </li>';
        }
        if (isset($_SESSION['Active'])) {
            echo '<li class="list-inline-item" data-toggle="tooltip" data-placement="top" title="'.$t->trans("Create new page").'">
            <a href="'.BASE_URL.'p/create-page" id="sk-newPage" class="btn btn-outline-secondary btn-sm" role="button"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
            </li>';
        }
        if (isset($_SESSION['Active'])) {
            echo '<li class="list-inline-item" data-toggle="tooltip" data-placement="top" title="'.$t->trans("Settings").'">
            <a href="'.BASE_URL.'admin" id="sk-admin" class="btn btn-outline-secondary btn-sm" role="button"><i class="fa fa-cog" aria-hidden="true"></i></a>
            </li>';
        }
        ?>
    </ul>
    ...
    <div id="search">
        ...
        <form id="form-search" action="<?= BASE_URL ?>search" method="post">
            <input type="search" name="search" minlength="5" value="" placeholder="<?= $t->trans('Type the keywords here'); ?>" autocomplete="off" required />
        </form>
    </div>
    ...
    ?>
    <?php 
        $activeTopics = (isset($_SESSION['Active'])) ? $pageModel->getUniqTopics() : $pageModel->getUniqPublishedTopics();
        
        $url = $_SERVER['REQUEST_URI'] ?? '';
        $parse = parse_url($url, PHP_URL_PATH);
        $explode = explode('/', (string) $parse);
        $filenameURL = end($explode);
        $topicURL = prev($explode);

        if (!is_null($activeTopics) && !empty($activeTopics)) {
            echo '<li>';
            foreach ($activeTopics as $topic) {
                $topicTitle = str_replace('-', ' ', $topic);
                $isActive = (isset($topicURL) && $topicURL === $topic);
                $activeClass = $isActive ? 'menu-active' : '';
                $showClass = $isActive ? 'show' : '';
                
                echo '<a href="#'.$topic.'-side-navigation" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle '.$activeClass.' ">'. ucfirst($topicTitle) .'</a>';
                echo '<ul class="collapse list-unstyled '.$showClass.' " id="'.$topic.'-side-navigation">';

                $pages = (isset($_SESSION['Active'])) ? $pageModel->getPagesByTopic($topic) : $pageModel->getPublishedPagesByTopic($topic);

                if (!empty($pages)) {
                    foreach($pages as $page) {
                        $isActivePage = (isset($filenameURL) && $filenameURL === $page['filename'] && isset($topicURL) && $topicURL === $page['topic']);
                        $activePageClass = $isActivePage ? 'class="menu-active"' : '';
                        
                        $filenameTitle = str_replace('-', ' ', $page['filename']);
                        $link = BASE_URL . 'p/' . $page['slug'];
                        echo '<li><a href="'.$link.'" '.$activePageClass.' >'.ucfirst($filenameTitle).'</a></li>';
                    }
                }
                echo '</ul>';
            }
    ...
    <div class="d-flex justify-content-center">
        <div class="switch-box">
            <div class="toggle-switch" onclick="window.location.assign('<?= BASE_URL ?>switch-theme')">
                <input type="checkbox" class="switch-theme" <?= ($theme === 'dark') ? '' : 'checked' ?>>
